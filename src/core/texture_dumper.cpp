...
bool TryMergingVRAMWrite(const VRAMWrite& vrw)
{
  ...
  const u32 max_merge_height = g_settings.texture_replacements.dump_textures_max_mergee_height;
...
ALWAYS_INLINE static u32 ConvertVRAMPixelForDumping(u16 color)
{
  if (color == 0)
    return 0;
  ...
  const u32 a =
    g_settings.texture_replacements.dump_vram_write_force_alpha_channel ? 255 : (((color >> 15) != 0) ? 255 : 128);
...
void AddPaletteToVRAMWrite(VRAMWrite* vrw, const VRAMWrite::PixelValue& pv)
{
  ...
  switch (pv.mode)
  {
    case GPUTextureMode::Palette4Bit:
    {
      ...
      palette.reserve(16);
      for (u32 i = 0; i < 16; i++)
        palette.push_back(ConvertVRAMPixelForDumping(base[i]));
    }
    break;
...
void DumpVRAMWriteForDisplay(u32 width, u32 height, const void* pixels)
{
  ...
  for (u32 y = 0; y < height; y++)
  {
    ...
    {
      image.SetPixel(x, y, ConvertVRAMPixelForDumping(*src_pixels));
      src_pixels++;
    }
  }
  ...
  switch (mode)
  {
    ...
    case GPUTextureMode::Direct16Bit:
    {
      for (u32 y = 0; y < height; y++)
      {
        ...
        for (u32 x = 0; x < width; x++)
        {
          image.SetPixel(x, y, ConvertVRAMPixelForDumping(vram[x]));
          resolved_pixels++;
        }
      }
    }
    ...
    for (u32 x = 0; x < width; x++)
    {
      ...
      while (rect.right < width)
      {
        for (u32 check_y = rect.top; check_y < rect.bottom; check_y++)
        {
          ...
          if (done[check_index] || vrw.palette_values[check_index] != pv)
            goto cancel_east;
        }
